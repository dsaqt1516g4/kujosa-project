...
public class EventDAOQuery {
    ...
    public final static String UPDATE_EVENT_QUERY = "update events set name = ifnull(?, name), startdate = ifnull(?, startdate), enddate = ifnull(?, enddate) where id = unhex(?)";
    ...
    public final static String CREATE_EVENT = "insert into events (id, userid, titol, text, lat, lon, startdate, enddate, ratio) values (unhex(?),unhex(?),?,?,?,?,?,?,?,?)";
