...
public interface UserDAOQuery {
    ...
    public final static String CREATE_USER = "insert into users (userid, username, password, fullname,email , image) values (UNHEX(?), ?, UNHEX(MD5(?)), ?, ?,?)";
    ...
    public final static String GET_USER_BY_ID = "select hex(u.userid) as userid, u.loginid, u.email, u.fullname from users u where id=unhex(?)";
    public final static String GET_USER_BY_USERNAME = "select hex(u.id) as id, u.username, u.email, u.name, u.image from users u where u.username=?";
