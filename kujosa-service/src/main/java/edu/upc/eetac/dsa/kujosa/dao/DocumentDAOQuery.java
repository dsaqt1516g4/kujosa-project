...
public interface DocumentDAOQuery {
    ...
    String GET_DOCUMENT_BY_ID = "select hex(d.id) as id, hex(d.userid) as userid, d.description, d.name, d.creation_timestamp, d.last_modified, u.fullname from document d, users u where d.id=unhex(?) and u.id=d.userid";
}
